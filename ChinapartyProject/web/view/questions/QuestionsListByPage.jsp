...
<body>
	<div class="x-body">
	    <div class="layui-row">
	      ...
	      <div class="layui-input-inline" id="search">
			...
			<input type="text" name="sname" id="sname" placeholder="请输入题目" autocomplete="off" class="layui-input">
	    ...
	    <xblock>
	      ...
	      <button class="layui-btn" onclick="x_admin_show('添加题目','<%=path %>/view/questions/Questions.jsp',700,400)"><i class="layui-icon"></i>添加</button>
    ...
    layui.use('laydate',function(){
	 ...
	 layui.use('table',function(){
		 ...
		 table.render({
			 elem:'#mytable'
			 ,url:'/ChinapartyProject/getAllQuestionsByPageServlet'
			 ,method:'post'
			 ,cols:[[
					 {type:'checkbox'}
					 ,{title:'序号',type:'numbers'}
					 // ,{title:'ID',hide:'questions_id'}
					 ,{title:'主题',field:'themeTitle',sort:true}
					 ,{title:'题干',field:'quesTitle',sort:true}
					 ,{title:'选项A',field:'quesA',sort:true}
					 ,{title:'选项B',field:'quesB',sort:true}
					 ,{title:'选项C',field:'quesC',sort:true}
					 ,{title:'选项D',field:'quesD',sort:true}
					 ,{title:'分值',field:'quesScore',sort:true}
					 ,{title:'标准答案',field:'quesStand',sort:true}
					 ,{title:'操作时间',field:'quesCDate',sort:true}
					 ,{title:'状态',field:'quesFlag',sort:true}
		  ...
		  table.on('tool(mytable)', function(obj){
		    ...
		    if(obj.event === 'del'){//恒等
		      layer.confirm('真的删除行么', function(index){
		    	  $.ajax({
	          		  type:'POST'
	          		  ,url:'/ChinapartyProject/deleteQuestionsServlet'
	          		  ,data:{"cid":data.questions_id}
	          	      ,success:function(data){
	          	    	  if(data=="Ok"){
	          	    		  layer.alert('删除成功',{icon:6});
	          	    		  obj.del();
		    	...
		    	x_admin_show('编辑','/ChinapartyProject/findQuestionstServlet?cid='+data.questions_id,700,400);
		    }
		  }); 
         //查询
         var active={
