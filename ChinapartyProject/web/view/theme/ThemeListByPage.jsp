...
<div class="x-body">
        ...
        <form class="layui-form layui-col-md12 x-so">
          ...
          <select name="cflag" id="cflag">
        	...
        	<option value="2">已测试</option>
	          ...
	          <input type="text" name=""ctitle"" id="ctitle" placeholder="请输入测试主题" autocomplete="off" class="layui-input">
      ...
      <xblock>
        ...
        <button class="layui-btn" onclick="x_admin_show('添加测试','<%=path %>/view/theme/Theme.jsp',700,400)"><i class="layui-icon"></i>添加</button>
    ...
    function member_stop(obj,id){
		...
		layui.use('table',function(){
			...
			table.render({
				elem:'#mytable'
				,url:'/ChinapartyProject/getAllThemeByPageServlet'
				,method:'post'
				,cols:[[
						{type:'checkbox'}
						,{title:'序号',type:'numbers'}
						// ,{title:'ID',hide:'theme_id'}
						,{title:'主题',field:'themeTitle',sort:true}
						,{title:'状态',field:'themeFlag',sort:true}
						
						,{title:'操作时间',field:'themeCDate',sort:true}
			  ...
			  table.on('tool(mytable)', function(obj){
			    ...
			    if(obj.event === 'del'){//恒等
			      layer.confirm('真的删除行么', function(index){
			    	  $.ajax({
		          		  type:'POST'
		          		  ,url:'/ChinapartyProject/deleteThemeServlet'
		          		  ,data:{"cid":data.theme_id}
			    	...
			    	x_admin_show('编辑','/ChinapartyProject/findThemeServlet?cid='+data.theme_id,700,400);
			    }
			  }); 
              //查询
              var active={
